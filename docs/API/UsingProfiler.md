...
Compositor: views 2 @ 2048x1984 | update 2.10ms | inputSlack 1.45ms | earlyStart 0.00ms | submit 3.20ms | semWait 0.00ms | deadlineMargin 2.35ms | presentMargin 6.80ms | missed 3/5400 (0.06%) | noAnchor 0
```
...
| `earlyStart` | How far ahead of `optimalInputTime` the submission phase started. Zero while frames make their deadline; otherwise `XRFramePacer` grows it to cover the miss plus a target margin (see the XR rendering system doc). It cannot exceed `inputSlack`. |
