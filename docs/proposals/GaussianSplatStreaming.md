...
| Streaming | Gaussian props share the tile streaming path: load and unload by radius, near band, `MemoryBudgetManager` registration by estimated GPU bytes, eviction under pressure, progressive tiers pulled on demand. Within an asset, row 10 pages `.untoldgs` chunks from disk into a bounded pool fed by the chunk cull. | `Systems/GeometryStreamingSystem+GaussianStreaming.swift` |
...
- The payload is one flat blob: no chunking, no byte-range reads, no spatial order or tree. Streaming inside one asset (the window and environment modes) is impossible, and every resident splat costs 48 B plus SH where a quantised record needs 16 B plus SH. (Rows 2, 5 and 8 fixed the format and the residency; row 10 pages the chunks of one asset from disk, so an asset above the paging threshold keeps a bounded pool of 256-rank tiers rather than its records.)
...
|---|---|---|---|---|---|
| Vision Pro M2 (2024) — floor | 11.1 ms · 2 eyes | 0.5–0.8 M per eye | 15–20 M | degree 2 or palette | `pread` page pool now (row 10); Metal IO behind the same `GaussianPageSource` later |
| Vision Pro M5 (2025) — target | 11.1 ms, or 8.3 ms at 120 Hz | 0.8–1.5 M per eye | 20–30 M | degree 3 or palette | `pread` page pool now; Metal IO later |
| Apple silicon Mac | 16.7 ms | 4–8 M | 30 M+ | degree 3 | whole-resident below 512 MiB, else the page pool; mmap or Metal IO later |
| iPad Pro | 8.3 / 16.7 ms | 2–4 M | 10–20 M | degree 2 or palette | `pread` page pool now; Metal IO later |
| iPhone | 16.7 ms | 1–2 M | 5–10 M | degree ≤ 2 or palette | `pread` page pool now; Metal IO later |
...
1. *Object gate.* Per splat entity: world AABB, padded frustum test, screen-height estimate, LOD pick from the budget table adjusted so the sum across visible objects fits the frame budget. Entities that fail are skipped entirely. (The screen-size estimate now exists per chunk: the chunk cull of row 9 computes each chunk's clipped screen area and the budget weights its quota by it.)
...
| A region snapping from blurry to sharp | Streaming popping | Coarse level always resident; arriving chunks fade in over 150–300 ms (in place since row 10: an arriving tier fades in over 16 frame-counted frames, and a paged tier warms before the LOD switch); prefetch by velocity look-ahead (open); hold the current level under a stall. |
...
| 8 | `feature/gaussian_budgeted_working_set` | [Feature] Budgeted working set fed by chunk-level culling | Shipped in two stages. Stage 1: the `.untoldgs` chunk table is retained on the component; `gaussianChunkCull` tests each chunk's centre AABB padded by `kGaussianQuadSigma·exp(logScaleMax)` against the guard-banded frustum (either eye in stereo, both rebuilt with the current scene root) and the previous frame's HZB; `GaussianDebugOptions.disableChunkCull`. Stage 2: the 16-byte records stay resident and the 48-byte encoded buffer and per-slot index buffers are gone for chunked entities; one fused pass per visible chunk (`gaussianChunkDecodePreprocess`) decodes, tests (either eye), projects and compacts into the shared set; the set is sized to `GaussianRuntimeLimits.workingSetSplats` (1 M mobile, 6 M Mac, clamped by the memory budget and the resident total, `workingSetSplatsOverride`); per-chunk quotas `floor(scale × count)` from a persistent `GaussianBudgetState` with asymmetric hysteresis (a fall at once, a rise by at most max(10 %, 0.05)/frame) and an opacity band over the last fifth of each truncated chunk; the shared set has its own `MemoryBudgetManager` entry; per-entity caps raised to 20 M / 40 M for the chunked path (the whole-buffer path keeps 5 M / 16 M); `disableWorkingSetBudget`. `.ply` entities keep the whole-buffer path, unbudgeted: the set never drops below their resident total and their visible counts are reserved before the chunked entities are fitted. | 6 |
| 9 | `feature/gaussian_screen_weighted_quotas` | [Feature] Per-chunk screen-size quota weighting | The chunk cull computes each visible chunk's clipped screen area in view units from the corner loop it already runs (the larger eye's in stereo, the guard-banded volume for a chunk reaching behind the eye) and bins its density into a 64-tier `GaussianBudgetDensityHistogram`; `gaussianComputeBudgetScale` solves one density cap per frame against the room the headroom leaves (never `scale × request`), with the scale's fall-at-once / rise-capped hysteresis and a sticky whole state, and `gaussianComputeChunkQuotas` grants `min(count, floor(cap × area))` — near chunks keep everything, far dense chunks are cut first — under the same `Σ quota ≤ 0.98 × budget − reserved` bound. `disableScreenWeightedQuotas` restores the uniform rule byte for byte. | 8 |
| 10 | `feature/gaussian_chunk_paging` | [Feature] Disk paging of `.untoldgs` chunks: page pool, cull-fed residency, rank-prefix tiers | Above a platform threshold (64 MiB mobile, 512 MiB Mac) an asset's records live in a bounded page pool of 256-rank tiers (`GaussianPageManager`, the pool is `packedSplatData`); per in-flight slot a residency table, a page table and a demand table; `gaussianChunkCull` writes every chunk's seen area into the demand table, skips chunks with nothing resident and lists the others with their resident ranks so the budget sees only drawable ranks; `gaussianChunkDecodePreprocess` maps ranks through the page table and fades arriving tiers in over 16 frames; the pager ticks before the cull — demand read back three frames later (a CPU seed on the first ticks), wanted ranks from the quota rule with a 1.25 headroom or a fill density, priority by area, deficit and tier, eviction of stale chunks, surplus tiers and displaced tails with hold-off, minimum residency, a 1.5× margin and a reload cooldown, `pread` on a background queue straight into retired slots reused three ticks later, CRC at full residency, backoff and faults, a faulted asset that reopens; a warmth gate before a paged tier's LOD switch; a memory-pressure soft target; `disablePaging`, `freezePaging`, `disablePageFade`, `residencyDebugTint`; `UntoldGSCRC32` slicing by eight with a streaming form. | 9 |

Phase 2 (tiers as chunk ranges in one file, merged coarse levels, object budgets, the window bake) and phase 3 (streamed environment) follow as their own series once PRs 1–7 are merged upstream. Row 10 shipped the disk paging within an asset (range loads, a page pool, residency); the next item — per-chunk LOD tiers (tiers as chunk ranges, merged coarse levels) — build on the per-chunk screen area row 9 introduces: it is the weight a pager and a tier pick need.
