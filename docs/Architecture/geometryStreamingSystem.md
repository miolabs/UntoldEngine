...
The same callbacks give every paged Gaussian splat's page pool a soft target through `GaussianPagePoolRegistry.noteMemoryPressure`: half its slots on `.warning`, a quarter on `.critical`, for `GaussianPagingPolicy.pressureTicks` (600 ticks); the pager evicts down to it (stale chunks, surplus tiers, then the cheapest tiers regardless of margin or pin) and issues no read that would carry it back above it. This bounds the touched footprint and the I/O, not the allocation — the pools are fixed `MTLBuffer`s freed with their entity — so whole-entity eviction remains the reliable relief. The OS callback fires on a background queue and sets a `pendingPressureRelief` flag on `GeometryStreamingSystem`. The flag is drained at the **start of the next `update()` tick** on the main thread, so all eviction work stays on the same thread as the rest of the streaming system. This prevents the OS from silently escalating to `.critical` and terminating the process — on visionOS in particular, the window between `.warning` and process kill can be under a second.
